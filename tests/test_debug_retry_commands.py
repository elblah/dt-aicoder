...
from aicoder.commands.debug_command import DebugCommand
from aicoder.commands.retry_command import RetryCommand
...
def test_handler():
    ...
    handler = TestHandler()
    # Create command instances with the handler
    debug_cmd = DebugCommand(handler)
    retry_cmd = RetryCommand(handler)

    # Add the handler as an attribute for the commands to access
    handler.debug_cmd = debug_cmd
    handler.retry_cmd = retry_cmd
...
def test_debug_show_status_disabled(test_handler, clean_env):
    ...
    result = test_handler.debug_cmd.execute([])
...
def test_debug_enable(test_handler, clean_env):
    ...
    result = test_handler.debug_cmd.execute(["on"])
...
def test_debug_show_status_enabled(test_handler, clean_env):
    ...
    # Enable debug first
    test_handler.debug_cmd.execute(["on"])
...
def test_debug_disable(test_handler, clean_env):
    ...
    result = test_handler.debug_cmd.execute(["off"])
...
def test_debug_invalid_argument(test_handler, clean_env):
    ...
    result = test_handler.debug_cmd.execute(["invalid"])
...
def test_retry_with_debug_disabled(test_handler, clean_env):
    ...
    result = test_handler.retry_cmd.execute([])
...
def test_retry_insufficient_messages(clean_env):
    ...
    class SystemMessageOnlyHandler:
        def __init__(self):
            self.message_history = MessageHistory()
    ...
    result = handler.retry_cmd.execute([])
...
def test_retry_with_various_debug_states(test_handler, clean_env):
    ...
    # Disable debug
    test_handler.debug_cmd.execute(["off"])
