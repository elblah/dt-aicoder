...
from aicoder.prompt_loader import (
    get_user_prompts_directory, list_available_prompts,
    load_prompt_from_file, _apply_prompt_variables
)
...
class TestPromptCommand(unittest.TestCase):
    ...
    def setUp(self):
        ...
        (self.prompts_dir / "001-gemini.txt").write_text("You are a Gemini AI assistant. {current_directory}")
        (self.prompts_dir / "qwen.md").write_text("You are a Qwen AI expert. Current user: {current_user}")
        (self.prompts_dir / "python-helper.md").write_text("You are a Python expert helper.")
    ...
    def test_handle_prompt_list(self):
        ...
        with patch('builtins.print') as mock_print:
            ...
            prompt_calls = [call for call in mock_print.call_args_list
    ...
    def test_handle_prompt_set_valid(self):
        ...
        mock_message_history.messages = [{'role': 'system', 'content': 'old prompt'}]
        handler.message_history = mock_message_history
        ...
        with patch('builtins.print') as mock_print:
            with patch.dict(os.environ, {}, clear=False):
                ...
                self.assertEqual(mock_message_history.messages[0]['role'], 'system')
                self.assertIn('You are a Gemini AI assistant', mock_message_history.messages[0]['content'])
    ...
    def test_handle_prompt_set_invalid_number(self):
        ...
        with patch('builtins.print') as mock_print:
            ...
            self.assertTrue(len(error_calls) > 0, "Should print error for invalid prompt number")
    ...
    def test_handle_prompt_set_no_prompts(self):
        ...
        with patch('builtins.print') as mock_print:
            ...
            self.assertTrue(len(error_calls) > 0, "Should print error when no prompts available")

    @patch('subprocess.run')
    @patch('builtins.input')
    @patch('tempfile.NamedTemporaryFile')
    def test_handle_prompt_edit_temp_file(self, mock_tempfile, mock_input, mock_subprocess):
        """Test the /prompt edit command with a temporary file."""
        handler = CommandHandlerMixin()
        handler.message_history = MagicMock()
        handler.message_history.messages = [{'role': 'system', 'content': 'old prompt'}]
        handler._run_editor_in_tmux_popup = MagicMock(return_value=False)  # Don't use tmux popup
        ...
        with patch('builtins.open', create=True) as mock_open:
            mock_open.return_value.__enter__.return_value.read.return_value = "Edited prompt content"
            mock_open.return_value.__enter__.return_value.write = MagicMock()

            with patch('pathlib.Path.mkdir'):
                with patch('pathlib.Path.write_text'):
                    with patch.dict(os.environ, {'AICODER_PROMPT_MAIN': ''}, clear=False):
    ...
    @patch('builtins.input')
    @patch('tempfile.NamedTemporaryFile')
    def test_handle_prompt_edit_cancelled(self, mock_tempfile, mock_input, mock_subprocess):
    ...
    def test_update_conversation_prompt(self):
        ...
        self.assertEqual(mock_message_history.messages[0]['content'], 'new prompt')
