    ...
    for injection_attempt in injection_attempts:
        ...
        if injection_marker in result and result != "No matches found":
            raise AssertionError(
                f"GREP TOOL VULNERABLE: Injection detected in output: {result}"
            )
...
def test_list_directory_tool_injection_security():
    ...
    for injection_attempt in injection_attempts:
        ...
        if injection_marker in result and "Directory not found" not in result:
            raise AssertionError(
                f"LIST_DIRECTORY TOOL VULNERABLE: Injection detected in output: {result}"
            )
    ...
    try:
        for injection_attempt in injection_attempts:
            ...
            if os.path.exists(temp_file):
                raise AssertionError(
                    f"ACTUAL COMMAND EXECUTION: File created via injection: {temp_file}"
                )
                os.remove(temp_file)
