...
def test_no_messages_to_compact_exception():
    ...
    with temp_config(config,
                     ...
                     AUTO_COMPACT_THRESHOLD=10):  # Very low threshold
        
        # Add messages that will all be protected (recent)
        message_history.add_user_message("Hello")
        message_history.add_assistant_message({"role": "assistant", "content": "Hi there"})
...
def test_exception_in_command_handler():
    ...
    handler.message_history = Mock()
    
    # Make compact_memory raise the exception
    handler.message_history.compact_memory.side_effect = NoMessagesToCompactError("Test error")
    handler.message_history._compaction_performed = False
    ...
    with contextlib.redirect_stdout(f):
        should_quit, run_api_call = handler._handle_summary([])
