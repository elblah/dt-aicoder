...
def test_executor_with_mocked_approval():
    ...
    with patch('aicoder.tool_manager.executor.config'), \
         patch('aicoder.tool_manager.executor.ApprovalSystem', return_value=mock_approval_system), \
         patch('aicoder.tool_manager.executor.INTERNAL_TOOL_FUNCTIONS', {"run_shell_command": mock_shell_func}):
        executor = ToolExecutor(mock_tool_registry, mock_stats, mock_animator)
...
def test_executor_command_info_printing():
    ...
    with patch('aicoder.tool_manager.internal_tools.run_shell_command.execute_run_shell_command') as mock_run:
