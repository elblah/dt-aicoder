...
def test_tool_registry_initialization():
    ...
    assert isinstance(registry.mcp_tools, dict)
...
def test_tool_manager_initialization():
    ...
    assert hasattr(manager, "approval_system")

    # Verify they are the correct types
    assert isinstance(manager.registry, ToolRegistry)
    assert isinstance(manager.executor, ToolExecutor)
    assert isinstance(manager.approval_system, ApprovalSystem)


def test_tool_manager_has_core_tools():
