...
def test_comprehensive_dangerous_patterns():
    ...
    try:
        ...
        for cmd, expected_reason_pattern in dangerous_cases:
            ...
            assert (
                "requires manual approval" in reason
                or "Dangerous pattern detected" in reason
            )
            # Check that the specific pattern is mentioned
            assert any(
                pattern in reason for pattern in expected_reason_pattern.split()
            ), (
    ...
    try:
        ...
        for cmd in safe_commands:
            ...
            assert result is True, (
                f"Command '{cmd}' should be safe (semicolons in Python strings)"
            )
        ...
        for cmd in dangerous_commands:
            result = validate_shell_command({"command": cmd})
            assert result is True, (
                f"Command '{cmd}' should proceed to normal approval (command separator)"
            )
    finally:
        config.YOLO_MODE = original_yolo
...
def test_edge_cases():
    ...
    try:
        ...
        assert isinstance(result, str), (
            "Whitespace-only command should return error message"
        )
        assert "Empty command" in result
    finally:
        config.YOLO_MODE = original_yolo
...
def test_simulated_aicoder_session_flow():
    ...
    try:
        ...
        approval_system = ApprovalSystem(
            MockToolRegistry(), MockStats(), MockAnimator()
        )
        ...
        session_commands = [
            ...
            (
                ...
                False,
                False,
                "python should ask for approval first time",
            ),
            ...
            (
                ...
                False,
                False,
                "python should still ask for approval",
            ),
            ...
            (
                ...
                True,
                False,
                "python should be auto-approved after session approval",
            ),
            ...
            (
                ...
                False,
                False,
                "python with semicolon should require approval despite session",
            ),
        ]
        ...
        for (
            ...
            assert main_command == expected_main_cmd, (
                f"Main command mismatch for '{cmd}'"
            )
            ...
            if has_dangerous:
                ...
                was_session_approved = (
                    cache_key in approval_system.tool_approvals_session
                )

                if should_auto_approve and not has_dangerous:
                    assert (
                        dynamic_config.get("auto_approved", False)
                        or was_session_approved
    ...
    try:
        edge_cases = [
            ...
            (
                ...
                "python",
                "Complex path should extract command correctly",
            ),
            (
                "./script.sh",
                "script.sh",
                "Relative path should extract command correctly",
            ),
            (
                "../python -V",
                "python",
                "Parent directory path should extract command correctly",
            ),
            # Commands with quotes and special characters
            (
                'python -c "print(\\"hello world\\")"',
                "python",
                "Python with escaped quotes should work",
            ),
            (
                "python -c 'print(\"hello\")'",
                "python",
                "Python with mixed quotes should work",
            ),
            (
                'bash -c "echo \\"test\\""',
                "bash",
                "Bash with nested quotes should work",
            ),
