...
def test_retry_handler_should_retry_502():
    ...
    # Enable test mode to disable singleton behavior and retry patterns
    original_test_mode = os.environ.get("AICODER_TEST_MODE", "")
    os.environ["AICODER_TEST_MODE"] = "1"

    # Disable retry patterns to use default logic
    ...
    error = urllib.error.HTTPError(
        url=f"{base_url}/502", code=502, msg="Bad Gateway", hdrs={}, fp=None
    )
...
def test_retry_handler_should_retry_500_with_429_content():
    ...
    original_test_mode = os.environ.get("AICODER_TEST_MODE", "")
    os.environ["AICODER_TEST_MODE"] = "1"
    # Disable retry patterns for this test to test original logic
    os.environ["DISABLE_RETRY_PATTERNS"] = "1"
    ...
    assert error_type == "Rate limiting", (
        "500 with 429 content should be classified as Rate limiting"
    )
    assert delay == aicoder.config.RETRY_INITIAL_DELAY, (
        "Rate limiting should use exponential backoff starting from initial delay"
    )
    ...
    # Clean up retry patterns environment variable
...
def test_retry_handler_should_not_retry_401():
    ...
    error = urllib.error.HTTPError(
        url=f"{base_url}/401", code=401, msg="Unauthorized", hdrs={}, fp=None
    )
...
def test_api_client_handle_http_error_max_attempts():
    ...
    with patch(
        "aicoder.retry_utils.cancellable_sleep", side_effect=mock_cancellable_sleep
    ):
        should_retry = handler.handle_http_error_with_retry(error)
        ...
        should_retry = handler.handle_http_error_with_retry(error)
        assert should_retry, (
            "Second attempt should still allow retry with default max attempts"
        )
        assert retry_count == 1, "Should have made 1 retry attempt"
        ...
        assert retry_count == 3, (
            "Should have made 3 retry attempts with default max attempts"
        )
...
def test_retry_with_multiple_error_codes():
    ...
    for code in error_codes_to_test:
        error = urllib.error.HTTPError(
            url=f"{base_url}/{code}", code=code, msg="Test Error", hdrs={}, fp=None
        )
...
def test_500_error_without_rate_limiting_content():
    ...
    error = urllib.error.HTTPError(
        url=f"{base_url}/500", code=500, msg="Internal Server Error", hdrs={}, fp=None
    )
    ...
    assert not should_retry, (
        "500 errors without rate limiting content should NOT be retried"
    )
    assert delay == 0, "Delay should be 0 for non-retryable errors"
    assert error_type == "Unknown", (
        "500 without rate limiting should be classified as Unknown error"
    )
...
def test_calculate_retry_delay_fixed():
    ...
    print(
        f"DEBUG: test_calculate_retry_delay_fixed - ENABLE_EXPONENTIAL_WAIT_RETRY = {aicoder.config.ENABLE_EXPONENTIAL_WAIT_RETRY}"
    )
    print(
        f"DEBUG: test_calculate_retry_delay_fixed - RETRY_FIXED_DELAY = {aicoder.config.RETRY_FIXED_DELAY}"
    )
    print(
        f"DEBUG: test_calculate_retry_delay_fixed - AICODER_TEST_MODE = {os.environ.get('AICODER_TEST_MODE', 'NOT_SET')}"
    )
    ...
    try:
        ...
        delay_1 = handler._calculate_retry_delay()
        print(
            f"DEBUG: test_calculate_retry_delay_fixed - delay_1 = {delay_1}, expected = {aicoder.config.RETRY_FIXED_DELAY}"
        )
        assert delay_1 == aicoder.config.RETRY_FIXED_DELAY, (
            f"Expected {aicoder.config.RETRY_FIXED_DELAY}, got {delay_1}"
        )

        handler.retry_attempt_count = 5
        delay_2 = handler._calculate_retry_delay()
        print(
            f"DEBUG: test_calculate_retry_delay_fixed - delay_2 = {delay_2}, expected = {aicoder.config.RETRY_FIXED_DELAY}"
        )
        assert delay_2 == aicoder.config.RETRY_FIXED_DELAY, (
            f"Expected {aicoder.config.RETRY_FIXED_DELAY}, got {delay_2}"
        )
    finally:
...
def test_request_success_endpoint():
    ...
    with patch.object(
        aicoder.config, "get_api_endpoint", return_value=f"{base_url}/success"
...
def test_request_502_error():
    ...
    # Use patch to temporarily change the config values just for this request
    with patch.object(
        aicoder.config, "get_api_endpoint", return_value=f"{base_url}/502"
    ), patch.object(aicoder.config, "get_api_key", return_value="test-key"):
        try:
...
def test_request_500_error():
    ...
    # Use patch to temporarily change the config values just for this request
    with patch.object(
        aicoder.config, "get_api_endpoint", return_value=f"{base_url}/500"
    ), patch.object(aicoder.config, "get_api_key", return_value="test-key"):
        try:
            temp_client._make_http_request(api_data, timeout=2)
            # If we reach here, the test failed
...
def test_request_429_error():
    ...
    # Use patch to temporarily change the config values just for this request
    with patch.object(
        aicoder.config, "get_api_endpoint", return_value=f"{base_url}/429"
    ), patch.object(aicoder.config, "get_api_key", return_value="test-key"):
        try:
            temp_client._make_http_request(api_data, timeout=2)
            # If we reach here, the test failed
...
def test_request_500_with_429_content():
    ...
    with patch.object(
        aicoder.config,
        "get_api_endpoint",
        return_value=f"{base_url}/500_with_429_content",
    ), patch.object(aicoder.config, "get_api_key", return_value="test-key"):
        try:
            temp_client._make_http_request(api_data, timeout=2)
            # If we reach here, the test failed
