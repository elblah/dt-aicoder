...
class TestExecutorIntegration:
    ...
    def test_approval_flow_integration(self):
        ...
        with patch.dict(
            ...
            with patch('aicoder.tool_manager.executor.config.YOLO_MODE', False):
                ...
                # Note: _handle_guidance_prompt method no longer exists in the implementation
                # This test just checks that the tool executes with approval
                message = {
                    "tool_calls": [
                        {
                            "id": "call_1",
    ...
    def test_file_tracking_integration(self):
        ...
        def mock_edit_tool(path: str, old_string: str, new_string: str, stats=None):
        ...
        with patch.dict(
            ...
            self.executor.tool_registry.message_history = Mock()
            with patch('aicoder.tool_manager.handlers.internal_handler.track_file_edit') as mock_track_edit, \
                 patch('aicoder.tool_manager.handlers.internal_handler.track_file_read') as mock_track_read:
    ...
    def test_complex_json_parsing_integration(self):
        ...
        with patch.dict(
            ...
            assert "Processed complex data" in results[0]["content"]
