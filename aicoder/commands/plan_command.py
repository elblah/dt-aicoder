...
class PlanCommand(BaseCommand):
    ...
    def _handle_help_command(self) -> Tuple[bool, bool]:
        ...
        print(f"  {config.BOLD}/plan{config.RESET}                    Show current planning mode status")
        print(f"  {config.BOLD}/plan toggle{config.RESET}            Toggle planning mode on/off")
        print(f"  {config.BOLD}/plan on{config.RESET}                Enable planning mode (read-only)")
        print(f"  {config.BOLD}/plan off{config.RESET}               Disable planning mode (read-write)")
        print(f"  {config.BOLD}/plan start{config.RESET}             Enable planning mode (alias)")
        print(f"  {config.BOLD}/plan end{config.RESET}               Disable planning mode (alias)")
        print(f"  {config.BOLD}/plan true{config.RESET}              Enable planning mode (alias)")
        print(f"  {config.BOLD}/plan false{config.RESET}             Disable planning mode (alias)")
        print(f"  {config.BOLD}/plan 1{config.RESET}                 Enable planning mode (alias)")
        print(f"  {config.BOLD}/plan 0{config.RESET}                 Disable planning mode (alias)")
        print(f"  {config.BOLD}/plan focus{config.RESET}             Create new session focused on last assistant message")
        print(f"  {config.BOLD}/plan help{config.RESET}              Show this help message")
        
        print(f"\n{config.GREEN}Description:{config.RESET}")
        print(f"  Planning mode provides read-only access to explore and analyze")
        print(f"  your codebase without making changes. When planning mode is")
        print(f"  enabled, destructive tools like write_file are disabled.")
        
        print(f"\n{config.GREEN}Focus Subcommand:{config.RESET}")
        print(f"  The {config.BOLD}/plan focus{config.RESET} command creates a new session with the")
        print(f"  last assistant message transformed into a user message.")
        print(f"  This is useful when the AI provides a good plan and you want")
        print(f"  to start a fresh session focused entirely on that plan.")
    ...
    def _handle_focus_command(self) -> Tuple[bool, bool]:
        ...
        if message_content:
            ...
            imsg(f"*** Focused on previous assistant message: {message_content[:100]}{'...' if len(message_content) > 100 else ''}")
        else:
            # If no content, add a placeholder
            self.app.message_history.add_user_message("Focus on previous assistant response")
