...
def remove_dimmed_pattern(pattern: str) -> bool:
    ...
    _dimmed_patterns = [
        p for p in _dimmed_patterns if p.pattern != pattern_str_to_remove
    ]
...
def initialize_dimmed_plugin():
    ...
    if debug:
        ...
        _original_print("   Use /dimmed command to configure")
...
def handle_dimmed_command(aicoder_instance, args):
    ...
    if subcommand == "help":
        print("\nDimmed Command Help:")
        print("  /dimmed                  - Show current patterns and status")
        print("  /dimmed add <pattern>    - Add a new pattern (temporary)")
        print("  /dimmed remove <pattern> - Remove a pattern (temporary)")
        print("  /dimmed clear            - Clear all patterns (temporary)")
        print("  /dimmed save             - Save current patterns to project config")
        print("  /dimmed save global      - Save current patterns to global config")
        print("  /dimmed reload           - Reload from config files")
        print("  /dimmed off              - Disable dimmed output")
        print("  /dimmed on               - Enable dimmed output")
        print("  /dimmed help             - Show this help")
        print("\nPattern examples:")
        print("  \\[.*?\\]                  - Match text in brackets [like this]")
        print("  \\(.*?\\)                  - Match text in parentheses (like this)")
        print("  Warning:.*               - Match lines starting with 'Warning:'")
        print("  \\bTODO\\b                - Match the word 'TODO'")
        print("  ^Error:.*               - Match lines starting with 'Error:'")
        print("  \\[DEBUG\\].*             - Match debug messages")
        return False, False
...
def on_aicoder_init(aicoder_instance):
    ...
    aicoder_instance.command_handlers["/dimmed"] = lambda args: handle_dimmed_command(
