...
def test_pattern_compilation():
    ...
    if len(compiled) != len(valid_patterns):
        print(f"❌ Expected {len(valid_patterns)} compiled patterns, got {len(compiled)}")
        return False
    ...
    if not matches:
        print(f"❌ Compiled patterns failed to match test string")
        return False
    ...
    if len(compiled_invalid) != 0:
        print(f"❌ Invalid patterns should not compile")
        return False
