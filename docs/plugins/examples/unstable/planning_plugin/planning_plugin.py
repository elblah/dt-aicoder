...
class PlanningPlugin:
    ...
    def _register_commands(self):
        ...
        if hasattr(self.aicoder, 'command_handlers'):
            ...
            self.aicoder.command_handlers['/approveplan'] = self._handle_approveplan_command
            
            if DEBUG:
                print(f"{GREEN}Planning commands registered: /plan, /endplan, /showplan, /approveplan{RESET}")
        else:
            print(f"{RED}Warning: Could not register planning commands - no command_handlers found{RESET}")
    ...
    def _handle_plan_command(self, args: List[str]) -> Tuple[bool, bool]:
        ...
        print(f"{YELLOW}I'll help you create a detailed plan before starting execution.{RESET}")
        print(f"{YELLOW}This ensures we understand all requirements and have clear steps.{RESET}\n")
    ...
    def _start_planning_conversation(self):
        ...
        system_index = next((i for i, msg in enumerate(original_messages) 
            ...
            try:
                ...
                if response and "choices" in response:
                    ...
                    if self._is_asking_questions(ai_response['content']):
                        ...
                        parsed_plan = self._parse_plan_from_response(ai_response['content'])
                        if parsed_plan:
...
Remember: Your goal is to create a clear, executable plan that another AI could follow."""
    ...
    def _is_asking_questions(self, content: str) -> bool:
        ...
        question_indicators = ['?', 'questions:', 'clarify', 'need more information', 'please provide']
            ...
            for line in lines:
                ...
                if line.startswith('# PLAN:'):
                    ...
                    if deps and deps.lower() != 'none':
                        try:
                            current_step.dependencies = [int(d.strip()) for d in deps.split(',') if d.strip()]
                        except ValueError:
                    ...
                    if tools and tools.lower() != 'none':
                        current_step.tools_needed = [t.strip() for t in tools.split(',') if t.strip()]
    ...
    def _display_plan(self):
        ...
        for step in self.current_plan.steps:
            ...
            if step.dependencies:
                print(f"     {YELLOW}Dependencies: {', '.join(map(str, step.dependencies))}{RESET}")
