...
SAFE_MODE_ENV_VAR = "AICODER_SAFE_TOOL_RESULTS"  # Environment variable to control safe mode
...
def tool_result_to_safe_text(tool_name: str, arguments: Dict[str, Any], result: Any) -> str:
...
def on_plugin_load():
    ...
    if ENABLED:
        print("✅ Safe tool results plugin loaded - converting tool outputs to text format")
    else:
...
def on_aicoder_init(aicoder_instance):
    ...
    try:
        ...
        # Apply safe wrappers
        ToolExecutor.execute_tool_calls = safe_execute_tool_calls_wrapper(_original_execute_tool_calls)
